...
LabRemote 在桌面程序内提供一个本机 Streamable HTTP MCP 服务，让支持 MCP 的客户端通过已保存的连接配置建立隔离隧道、查询状态、执行 SSH 命令或操作 MCP 专属交互终端。
...
- 只显示明确授权给 MCP 的连接配置；
- 隧道密码、SSH 密码和服务器密钥不会作为 MCP 参数或返回值出现；
- 当前 MCP 提供连接、命令和交互终端工具，不提供文件上传或下载工具；文件传输请使用 LabRemote 图形界面的“传输”窗口；
- 工具名中的 `vpn_connect`、`vpn_disconnect` 是兼容名称，实际操作的是 LabRemote 进程内 SoftEther 隔离隧道，不会连接或修改 Windows VPN。

## 2. 首次启用

### 2.1 先完成图形界面首次连接

首次连接某台服务器时，LabRemote 会要求确认 SoftEther 服务器证书和 SSH 主机密钥指纹。MCP 没有接受指纹的工具，因此应先在图形界面连接一次，并与管理员核对指纹后确认。
...
| 允许断开隔离隧道 | 允许请求断开进程内隔离隧道 | `vpn_disconnect` |
...
文档不会包含隧道密码、SSH 密码或 Credential Manager 凭据引用，但会包含当前 MCP Token。它等同于本机访问凭据，只能提供给同一台电脑上的受信任 AI 客户端。

开启状态和端口会保存到当前 Windows 用户配置中。下次启动 LabRemote 时，如果上次保持开启，程序会尝试恢复 MCP 服务。端口已被其他程序占用时，恢复或手动开启会失败。
...
5. 再次调用 `connection_status`，确认 `vpn_status` 为 `connected`、`route_ready` 为 `true` 且 `ssh_connected` 为 `true`；
...
`vpn_status` 中的“VPN”只是兼容命名，表示 LabRemote 进程内隔离隧道状态。Windows 的 VPN、路由和网卡不会因此改变。
...
| 字段 | 含义 |
|---|---|
| `vpn_status` | `disconnected`、`preparing`、`dialing`、`connected`、`reconnecting`、`disconnecting` 或 `failed` |
...
使用 Windows Credential Manager 中已保存的凭据建立进程内隔离隧道和 SSH 连接。
...
执行一条非交互远端命令。必须启用“允许执行非交互命令”。调用时会自动确保隔离隧道和 SSH 已连接。
...
- Token 保存在 Windows Credential Manager，不写入普通 JSON 配置；
...
访问令牌和 SSH/VPN 密码位于 Windows Credential Manager，不在上述文件中。
...
| `SSH_AUTH_FAILED` | 已保存 SSH 密码或用户名错误 | 在连接编辑页更新 SSH 凭据 |
